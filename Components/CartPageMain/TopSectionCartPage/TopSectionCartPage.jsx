      ...
      <div className="containerTopSectionCartPage">
        ...
        <div className="topHeaderTopSectionCartPage">
          ...
          <div className="headerRightTopSectionCartPage">
            <a href="/" className="continueShoppingTopSectionCartPage">
        ...
        <div className="contentWrapperTopSectionCartPage">
          ...
          <div className="leftColTopSectionCartPage">
            <div className="cartTableTopSectionCartPage">
              ...
              <div className="tableBodyTopSectionCartPage">
                {cartItemsCartSidebar.length === 0 ? (
                    <div style={{ padding: '20px', textAlign: 'center', width: '100%' }}>Your cart is empty.</div>
                ) : (
                    cartItemsCartSidebar.map((item) => (
                    <div key={item.idCartSidebar} className="cartRowTopSectionCartPage">
                        <div className="tdProductTopSectionCartPage">
                        <div className="productImageWrapperTopSectionCartPage">
                            <img src={item.imageCartSidebar} alt={item.titleCartSidebar} className="productImgTopSectionCartPage" />
                        </div>
                        <div className="productInfoTopSectionCartPage">
                            <h3 className="productNameTopSectionCartPage">{item.titleCartSidebar}</h3>
                            <p className="productTypeTopSectionCartPage">{item.type || 'Product'}</p>
                            <ul className="productDetailsListTopSectionCartPage">
                                {item.optionsCartSidebar && (
                                    <li>{item.optionsCartSidebar}</li>
                                )}
                            </ul>
                        ...
                        <div className="tdPriceTopSectionCartPage">{formatPrice(item.priceCartSidebar)}</div>
                        
                        <div className="tdQtyTopSectionCartPage">
                        <div className="qtySelectorTopSectionCartPage">
                            <button className="qtyBtnTopSectionCartPage" onClick={() => decreaseItemCartSidebar(item.idCartSidebar)}>-</button>
                            <span className="qtyValTopSectionCartPage">{item.quantityCartSidebar}</span>
                            <button className="qtyBtnTopSectionCartPage" onClick={() => increaseItemCartSidebar(item.idCartSidebar)}>+</button>
                        </div>
                        </div>
                        
                        <div className="tdTotalTopSectionCartPage">{formatPrice(item.priceCartSidebar * item.quantityCartSidebar)}</div>
                        
                        <div className="tdActionTopSectionCartPage">
                        <button className="removeBtnTopSectionCartPage" onClick={() => removeItemCartSidebar(item.idCartSidebar)}>
            ...
            <div className="cartFooterTopSectionCartPage">
              ...
              <a href="#" className="clearCartLinkTopSectionCartPage" onClick={(e) => { e.preventDefault(); cartItemsCartSidebar.forEach(item => removeItemCartSidebar(item.idCartSidebar)); }}>Clear Cart</a>
            </div>
          </div>
